CREATE DATABASE IF NOT EXISTS homework_04;
USE homework_04;
...
имя, фамилию и количество друзей cnt. Сортировка выводимых записей в порядке возрастания идентификатора пользователя.
*/
SELECT 
	u.id, 
	u.firstname, 
...
FROM users AS u
LEFT JOIN friend_requests fr ON (u.id = fr.target_user_id OR u.id = fr.initiator_user_id) AND fr.`status` = 'approved'
GROUP BY u.id;
