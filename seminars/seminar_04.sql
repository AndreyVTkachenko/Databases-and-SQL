DROP DATABASE IF EXISTS seminar_04;
...
-- вывести всех учителей, которые ведут предметы
-- INNER JOIN = JOIN (сокращённая форма)
SELECT
	t.surname,
    l.course
FROM teacher t
...
-- вывести абсолютно всех учителей, даже если они ничено не ведут
SELECT
	t.surname,
    l.course
FROM teacher t -- левая таблица
LEFT JOIN lesson l -- LEFT OUTHER JOIN = LEFT (сокращенная форма)
ON t.id = l.teacher_id;

-- вывести всех учителей, которые не ведут предметы
SELECT
	t.surname,
    l.course
FROM teacher t -- левая таблица
...
-- вывести всех учителей, которые ведут "Веб-технологии"
SELECT
	t.surname,
    l.course
FROM teacher t
JOIN lesson l -- INNER JOIN = JOIN (сокращенная форма)
ON t.id = l.teacher_id
WHERE l.course = "Знакомство с веб-технологиями";

-- ПОДЗАПРОСЫ
SELECT *
FROM seminar_03.staff;

-- Вывести должности, средние ЗП которых выше средней ЗП по пре-ю
SELECT 
	AVG(salary)
FROM seminar_03.staff; -- 40923.076923 средняя зарплата по предприятию
...
FROM seminar_03.staff
GROUP BY post
HAVING AVG(salary) > (SELECT AVG(salary) FROM seminar_03.staff);
...
-- выборка данных по пользователю: ФИО, город, название аватарки
SELECT
	u.firstname,
    u.lastname, 
    p.hometown AS city, -- город проживания
    m.filename AS photo_name
FROM users u -- users = u
JOIN `profiles` p ON u.id = p.user_id
JOIN media m ON p.photo_id = m.id;

-- Выборка данных по пользователю: ФИО, город, название всех фотографий 
SELECT
	u.firstname,
    u.lastname, 
    p.hometown AS city, -- город проживания
    m.filename AS photo_name
FROM users u -- users = u
JOIN profiles p ON u.id = p.user_id -- профиль
JOIN media m ON u.id = m.user_id -- получили все МЕДИА (это не только фото)
JOIN media_types mt ON mt.id = m.media_type_id
WHERE mt.id = (SELECT id FROM media_types WHERE name_type = "Photo"); -- останутся все фотки: аватарки + фото со страницы

-- список медиафайлов конкретного человека с указанием количества лайков и вывести топ-3 по популярности
